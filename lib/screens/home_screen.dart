...
class _HomeScreenState extends State<HomeScreen> {
  ...
  String? _savedEmployeeName;

  @override
  void initState() {
    super.initState();
    _loadSavedEmployeeName();
  }

  Future<void> _loadSavedEmployeeName() async {
    final preferences = await SharedPreferences.getInstance();
    if (!mounted) return;
    setState(
      () =>
          _savedEmployeeName = preferences.getString('selected_employee_name'),
    );
  }

  Future<void> _saveEmployeeName(String? name) async {
    if (name == null) return;
    setState(() => _savedEmployeeName = name);
    final preferences = await SharedPreferences.getInstance();
    await preferences.setString('selected_employee_name', name);
  }
  ...
  Future<void> _scanRota() async {
    ...
    try {
      ...
      setState(() {
        ...
        _selectedName = _preferredEmployeeName(_foundNames);
        _isProcessing = false;
      });
    } catch (error) {
      setState(() {
  ...
  String? _preferredEmployeeName(List<String> names) {
    if (names.isEmpty) return null;
    final saved = _savedEmployeeName;
    if (saved == null) return names.first;
    for (final name in names) {
      if (name.toLowerCase() == saved.toLowerCase()) return name;
    }
    return names.first;
  }
